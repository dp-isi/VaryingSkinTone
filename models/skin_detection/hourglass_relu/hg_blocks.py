...
def create_hourglass_network(num_classes, num_stacks, num_channels, inres, outres, bottleneck,inchannel):
    input = Input(shape=(inres[0], inres[1], inchannel))
    ...
    for i in range(num_stacks):
        head_next_stage, head_to_loss = hourglass_module(head_next_stage, num_classes, num_channels, bottleneck, i,[masked_person])
...
def create_front_module(input, num_channels, bottleneck,name_code='default'):
    ...
    _x = Conv2D(64, kernel_size=(7, 7), strides=(1,1), padding='same', activation='relu', name='front_conv_1x1_x1_%s'%(name_code))(
        input)
    _x = BatchNormalization()(_x)
...
def create_heads(prelayerfeatures, rf1, num_classes, hgid, num_channels,list_inputs):
    ...
    output_img = Conv2D(1, kernel_size=(1, 1), activation='sigmoid', padding='same',kernel_regularizer=regularizers.l1(0.01),
                        name=str(hgid)+'out_im' + '_conv_1x1_parts')(head)
